...
class DistanceBiasedTemporalAttention(nn.Module):
    def __init__(self, dim, num_heads=1, lambda_init=1.75):
        super().__init__()
...
class FeatureCache:
    ...
    def get_or_compute(self, video_dir, frame_paths, label):
        ...
        if os.path.exists(out_path):
            ...
            cached_video_dir = data.get("video_dir", None)

            if cached_label != expected_label:
                print(f"[CHECK B][WARN] Cache label mismatch for {out_path}")
                print(f"  cached_label={cached_label} expected_label={expected_label}")
                print(f"  current_video_dir={video_dir}")
                print(f"  cached_video_dir={cached_video_dir}")
                raise RuntimeError("Cache label mismatch detected — old cache or dataset mapping conflict.")

            if cached_video_dir is not None and os.path.abspath(cached_video_dir) != os.path.abspath(video_dir):
                raise RuntimeError(f"Cache path collision detected for {video_dir}")
...
def main():
    ...
    cache_dir = "/scratch/vsf6tk/suturing/results/videomae_feature_cache_knot_binary"
    ...
    run_experiment(
        exp_name="binary_knot_OHSK_vs_THSK_distance_lambda1.75",
